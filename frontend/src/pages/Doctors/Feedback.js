import React, { useState } from "react";
import { AiFillStar } from "react-icons/ai";
// import avatar from "../../assets/images/images.png";
import { formateDate } from "../../utils/formateDate";
import FeedbackForm from "./FeedbackForm";

const Feedback = ({ reviews, totalRating }) => {
  const [showFeedbackFrom, setShowFeedbackFrom] = useState(false);

  return (
    <div>
      <div className="mb-[50px]">
        <h4 className="text-[20px] leading-[30px] font-bold text-darkColor">
          All reviews ({totalRating})
        </h4>

        {reviews?.map((review, index) => (
          <div key={index} className="flex justfy-between p-3 gap-10 mb-[30px]">
            <div className="flex gap-3">
              <figure className="w-10 h-10 rounded-full">
                <img className="w-full" src={review?.user?.photo} alt="" />
              </figure>

              <div>
                <h5 className="test-[16px] leading-6 text-primaryColor font-bold">
                  {review?.user?.name}
                </h5>
                <p className="text-[14px] leading-6 text-gray-800">
                  {formateDate(review?.createdAt)}
                </p>
                <p className="textPara mt-3 font-medium text-[15px]">
                  {review.reviewText}
                </p>
              </div>
            </div>

            <div className="flex gap-1 ">
              {[...Array(review?.rating).keys()].map((_, index) => (
                <AiFillStar keys={index} color="#0067ff" />
              ))}
            </div>
          </div>
        ))}
      </div>

      {!showFeedbackFrom && (
        <div className="text-center">
          <button className="btn" onClick={() => setShowFeedbackFrom(true)}>
            Give Feedback
          </button>
        </div>
      )}

      {showFeedbackFrom && <FeedbackForm />}
    </div>
  );
};

export default Feedback;
